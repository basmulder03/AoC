...
{
    public static int RunOnce(IAoCDay dayImpl, string input, int? part, bool timingEnabled)
    {
        double ms1 = 0;
        ...
        if (timingEnabled)
            Console.WriteLine($"Total time: {(ms1 + ms2):F3} ms");
        
        return 0;
    }
    
    public static int RunBenchmark(IAoCDay dayImpl, int year, int day, string input, int? part, int iterations)
    {
